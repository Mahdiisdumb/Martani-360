    ...
    void Update()
    {
        if(inputs.climbLeft.action.ReadValue<float>() >= values.climbInput && lCC.inCol)
        {
            if(!other.isLeftGrabPointSet)
            {
                ...
                other.lp_RB.linearVelocity = Vector3.zero;
        ...
        else
        {
            if(!other.leftLaunch)
            {
                other.lp_RB.linearVelocity = (other.lp_RB.rotation * -inputs.veloLeft.action.ReadValue<Vector3>());
        ...
        else
        {
            ...
            {
                other.lp_RB.linearVelocity = (other.lp_RB.rotation * -inputs.veloRight.action.ReadValue<Vector3>());
